...
def scrape_kaisai_date(from_: str, to_: str, save_dir: Path = None) -> list[str]:
    ...
    for date in tqdm(pd.date_range(from_, to_, freq="MS")):
        ...
        soup = BeautifulSoup(html, "lxml", from_encoding="ecu-jp")
...
def scrap_html_horse(horse_id_list: list[str], save_dir: Path = HTML_HORSE_DIR, skip: bool = True) -> list[Path]:
    """
    netkeiba.comのhorseページのhtmlをスクレイピングして、save_dirに保存する関数。
    sikp=Trueにすると、既にhtmlが存在する場合はスキップされ、逆に上書きしたい場合は、skip=Falseにする。
    ...
    for horse_id in tqdm(horse_id_list):
        ...
        # binファイルが既に存在し、sikp=Trueの場合はスキップする
